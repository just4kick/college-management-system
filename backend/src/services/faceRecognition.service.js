...
// API URLs
const FACE_ENCODING_URL = process.env.FEU;
const FACE_VERIFY_URL = process.env.FVU;
console.log(process.env.FEU);
/**
...
const generateFaceEncoding = async (imagePath) => {
  ...
  try {
    ...
    throw new ApiError(
      500,
      `Face encoding failed: ${error.response?.data?.error || error.message}`
    );
  }
};
 ...
 */
const verifyFaceEncoding = async (
  imagePath,
  referenceEncoding,
  threshold = 0.7
) => {
  ...
  try {
    ...
    formData.append(
      "referenceEncoding",
      JSON.stringify(cleanedReferenceEncoding)
    );
    ...
    if (
      !response.data ||
      typeof response.data.isMatch !== "boolean" ||
    ...
    throw new ApiError(
      500,
      `Face verification failed: ${error.response?.data?.error || error.message}`
    );
  }
};
 ...
 */
const verifyAndRespond = async (
  imagePath,
  referenceEncoding,
  threshold = 0.7
) => {
  try {
    const { isMatch } = await verifyFaceEncoding(
      imagePath,
      referenceEncoding,
      threshold
    );
...
export { generateFaceEncoding, verifyFaceEncoding, verifyAndRespond };
